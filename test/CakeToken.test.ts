...
describe('CakeToken', function () {
  ...
  it('should fail if you try to do bad transfers', async function () {
    await mint(this.cake)(this.alice.address, '100');
    await expect(this.cake.transfer(this.carol.address, '110')).to.be.revertedWith(
      'BEP20: transfer amount exceeds balance'
    );
    await expect(
      this.cake.connect(this.bob).transfer(this.carol.address, '1', { from: this.bob.address })
